...
export default function DashboardPage() {
  ...
  const [hourlyWage, setHourlyWage] = useState(0) // 📌 DB 필드명 일치: hourly_wage
  ...
  useEffect(() => {
    const initDashboard = async () => {
      ...
      // 📌 profiles 테이블에서 hourly_wage를 정확히 가져옵니다.
      ...
      if (profile) {
        setHourlyWage(profile.hourly_wage || 0)
  ...
  const handleConditionSelect = async (status: string) => {
    ...
    try {
      ...
      if (response.ok) { setView('REPORT') } 
        ...
        alert('시스템 연결에 실패했습니다. 잠시 후 다시 시도하십시오.')
        setView('CONDITION') 
      }
    } catch (err) { 
      setView('CONDITION') 
      ...
      <header className="absolute top-0 left-0 w-full px-8 py-10 z-40 flex justify-between items-center">
        ...
        <div className="hidden md:flex bg-white/5 backdrop-blur-xl rounded-full p-1 border border-white/5 text-[9px] tracking-[0.2em] uppercase">
          {['SESSION', 'DIRECTIVE', 'REPORT', 'SETTINGS'].map(tab => (
            <button 
              key={tab} 
              onClick={() => tab === 'SETTINGS' ? router.push('/dashboard/billing') : setActiveTab(tab)} 
              className={`px-6 py-2.5 rounded-full transition-all duration-500 ${activeTab === tab ? 'bg-white/10 text-white' : 'text-zinc-600 hover:text-zinc-300'}`}
            >
              {tab}
            </button>
          ))}
      ...
      <AnimatePresence mode="wait">
        ...
        {view === 'CONDITION' && (
          <motion.div key="c" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="z-10 w-full flex flex-col items-center max-w-4xl">
            ...
            <h2 className="text-4xl md:text-6xl font-light tracking-tight text-white mb-24 leading-tight break-keep">
              대표님, 오늘 뇌의 컨디션을<br />
              <span className="text-[#C2A35D] font-serif italic font-bold">솔직하게</span> 알려주십시오.
            </h2>
            <div className="flex flex-col md:flex-row gap-5 mb-20 w-full max-w-2xl px-4">
              {['피곤함', '보통', '아주 좋음'].map((status) => (
                <button 
                  ...
                  className="flex-1 py-10 border border-zinc-900 bg-white/[0.01] hover:bg-white/[0.03] hover:border-[#C2A35D]/50 transition-all duration-500 rounded-2xl text-zinc-500 hover:text-white text-[13px] tracking-widest font-bold uppercase"
                >
                  {status}
                </button>
              ))}
          ...
          <motion.div key="r" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="z-10 w-full max-w-2xl px-4">
            <div className="bg-[#080808] border border-zinc-900 rounded-[40px] p-12 md:p-16 space-y-14 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-6 text-left">
                ...
                <h3 className="text-3xl md:text-4xl font-light leading-tight text-white break-keep">
                  {condition === '피곤함' 
                    ? '모든 결정을 중단하십시오. 오늘은 바탕화면의 폴더 하나만 정리하고 즉시 뇌를 휴식 상태로 전환하십시오.' 
                    : '최상의 인지 효율 상태입니다. 지금 바로 핵심 사업의 실행 계획 3가지만 물리적 동사로 확정하십시오.'}
                </h3>
              </div>
              <button 
                onClick={handleComplete} 
                className="w-full py-6 bg-white text-black text-[13px] font-bold tracking-[0.3em] uppercase hover:bg-[#C2A35D] transition-all duration-500 rounded-xl shadow-xl"
              >
                지침 완수 및 세션 종료
              </button>
            </div>
        ...
        {view === 'DONE' && (
          <motion.div key="d" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="z-10 text-center space-y-10">
              ...
              <p className="text-zinc-500 text-sm font-light tracking-[0.4em] leading-relaxed uppercase">
                오늘의 모든 결정 권한 행사가 종료되었습니다.<br />
                이제 뇌를 완전히 비우고 본질에 몰입하십시오.
              </p>
              <div className="w-20 h-[1px] bg-[#C2A35D] mx-auto opacity-30"></div>
          </motion.div>
        )}
