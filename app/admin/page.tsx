              ...
              <AnimatePresence mode="wait">
                ...
                {previewView === 'WELCOME_BACK' && (
                  <motion.div key="p-wb" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="z-10 w-full max-w-2xl px-6 flex flex-col items-center justify-center flex-1">
                    ...
                    <button onClick={() => setPreviewView('CONDITION')} className="px-16 py-7 border border-[#C2A35D] text-[#C2A35D] text-[14px] font-bold tracking-[0.25em] uppercase hover:bg-[#C2A35D] hover:text-black transition-all duration-500 rounded-xl">오늘의 설계 시작하기</button>
                  </motion.div>
                )}
                ...
                {previewView === 'CONDITION' && (
                  <motion.div key="p-c" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="z-10 w-full flex flex-col items-center justify-center max-w-4xl flex-1 px-6">
                    <div className="border border-[#C2A35D]/40 text-[#C2A35D] px-10 py-3 rounded-full text-[12px] font-bold tracking-[0.25em] mb-12 uppercase bg-[#C2A35D]/5">05:00 AM</div>
                ...
                {previewView === 'REPORT' && (
                  <motion.div key="p-r" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="z-10 w-full max-w-3xl px-6 pb-24 pt-6">
                    <div className="bg-[#080808] border border-zinc-800 rounded-3xl p-10 md:p-14 space-y-12 shadow-2xl relative overflow-hidden">
                      <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#C2A35D]/40 to-transparent"></div>
                      <div className="flex justify-between items-center border-b border-zinc-800/60 pb-8">
                      ...
                      <div className="space-y-10 text-left">
                        <div className="space-y-5">
                          ...
                          <div className="bg-[#111111] border border-zinc-800 rounded-2xl p-8 md:p-10">
                            <p className="text-base md:text-lg font-light leading-[1.7] text-zinc-100 tracking-wide break-keep">
                        ...
                        <div className="space-y-5">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div className="bg-[#111111] border border-zinc-800 rounded-2xl p-8 space-y-4">
                              <p className="text-zinc-400 text-[14px] flex items-center gap-3 tracking-wide">
                                <span className="bg-white text-black rounded-full w-6 h-6 flex items-center justify-center text-[12px] font-extrabold shadow-[0_0_12px_rgba(255,255,255,0.3)]">O</span> 하면 얻는 것
                              </p>
                            ...
                            <div className="bg-[#111111] border border-[#C2A35D]/20 rounded-2xl p-8 space-y-4 shadow-[0_0_15px_rgba(194,163,93,0.05)]">
                              <p className="text-zinc-400 text-[14px] flex items-center gap-3 tracking-wide">
                                <span className="bg-[#C2A35D] text-black rounded-full w-6 h-6 flex items-center justify-center text-[12px] font-extrabold shadow-[0_0_12px_rgba(194,163,93,0.4)]">X</span> 미루면 잃는 것
                              </p>
                        ...
                        <div className="space-y-5">
                          ...
                          <p className="text-zinc-400 text-[14px] md:text-[15px] font-light leading-[1.8] italic tracking-wide break-keep pr-4">
                            {condition === '피곤함' ? '"지금 버리지 못하는 그 한 가지가, 고객이 아닌 당신의 불안감을 채우기 위한 방어기제는 아닙니까?"' : '"당신의 1시간은 수십만 원입니다. 지금 고민하는 그 일이 그 이상의 가치를 창출합니까?"'}
                          </p>
                        </div>
                      </div>
                      <motion.button onClick={handleTestCheckIn} disabled={isCheckingIn} whileTap={{ scale: 0.96 }} className="w-full py-6 mt-8 bg-white text-black text-[14px] font-bold tracking-[0.3em] uppercase hover:bg-[#C2A35D] transition-colors duration-300 rounded-xl shadow-xl flex justify-center items-center gap-4 relative overflow-hidden cursor-pointer">
                ...
                {previewView === 'VALUE' && (
                  <motion.div key="p-v" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, y: -20 }} className="z-10 w-full max-w-2xl px-6 flex flex-col items-center">
                    <div className="w-20 h-20 border border-[#C2A35D] rounded-full flex items-center justify-center mb-10 bg-[#C2A35D]/5 shadow-[0_0_30px_rgba(194,163,93,0.1)]"><span className="text-[#C2A35D] text-3xl font-light">✓</span></div>
                    ...
                    <div className="w-full bg-[#080808] border border-zinc-800 rounded-3xl p-14 flex flex-col items-center justify-center space-y-8 relative overflow-hidden shadow-2xl mb-10">
                      ...
                      <div className="flex items-baseline gap-3">
                        ...
                        <span className="text-6xl md:text-8xl font-bold tracking-[-0.08em] text-white">{(hourlyWage * 150).toLocaleString()}</span>
                      </div>
                ...
                {previewView === 'DONE' && (
                  <motion.div key="p-d" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="z-10 text-center flex flex-col items-center justify-center w-full max-w-3xl px-6">
                    <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.8, ease: "easeOut" }} className="w-24 h-24 border border-[#C2A35D] rounded-full flex items-center justify-center mb-14 bg-[#C2A35D]/5 shadow-[0_0_50px_rgba(194,163,93,0.2)]"><span className="text-[#C2A35D] text-4xl font-light">✓</span></motion.div>
                    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.8 }} className="space-y-10 w-full">
                      <h2 className="text-3xl md:text-4xl font-serif font-bold text-white tracking-tight break-keep">오늘의 전략적 셧다운을 승인합니다.</h2>
                      <div className="text-zinc-400 text-[15px] md:text-[16px] font-light leading-[1.6] tracking-wide break-keep space-y-1">
                        <p>대표님은 방금 가장 핵심적인 과업을 해치웠습니다.</p>
                        <p>지금부터 머릿속에 떠오르는 '더 해야 하지 않을까?'라는 생각은,</p>
                        <p>완벽주의가 만들어낸 가짜 불안이자 내일의 에너지를 갉아먹는 '과잉'입니다.</p>
                        <div className="pt-8 mt-8 border-t border-zinc-800/80"><p className="text-white text-[17px] md:text-[18px] font-medium tracking-wide">더 이상의 실행을 통제하십시오.</p></div>
                      </div>
                    </motion.div>
                  </motion.div>
                )}
